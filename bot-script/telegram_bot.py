...
class ChatGPTTelegramBot:
    ...
    async def help(self, update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
        await update.message.reply_text(
            message_thread_id=get_thread_id(update),
            text='''После ввода команды /role вы пишете условия, которые нейросеть должна соблюдать. Например, если нужны краткие ответы без пояснений, можно попросить ИИ отвечать только "да" или "нет".
...
Если возникли вопросы, столкнулись с ошибкой напиши нам brainshtorm@gmail.com''',
    ...
    async def start(self, update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not await self.db.user_exists(user_id):
            ...
            await update.message.reply_text(
                message_thread_id=get_thread_id(update),
                text="Добро пожаловать. Активирован пробный период на 3 дня. Вам доступна модель GPT-3.5 и 2000 токенов",
            )
            return
...
Подробнее на сайте: brainstormai.ru''',
    ...
    async def role(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            message_thread_id=get_thread_id(update),
            text='Введите роль или введите /cancel для отмены',
        )
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                ...
                                url='https://brainstormai.ru/privacy-policy'
                            )
                        ]
                    ]
